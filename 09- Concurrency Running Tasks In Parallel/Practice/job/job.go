...
func (job *Job) Process(channel chan bool , erroChan chan error) {

	// Read prices from file
	err := job.loadPrices()
	if err != nil {
		fmt.Println("Error loading prices:", err)
		erroChan <- errors.New("failed to load prices " + err.Error())
		return  // to stop execution for the rest of the code
	}
	...
	channel <- true // signal that the job is done (only write on channel when job is done)
